 ...
 *
 * Мета: Створити функцію, що дозволяє легко генерувати нові об'єкти користувачів з переданими атрибутами
 * ім'я, вік та статус активності, використовуючи визначений тип User.
 *
 * Вимоги до реалізації:
...
const declaration: { [key in OrderStatus]: string } = {
...
function getOrderStatusSecond(status: OrderStatus): string {
  const result = declaration[status];
  if (!result) throw new Error("Невідомий статус замовлення");
  return result;
}
