...
const sumUsersBalance = (users) => {
    return users.reduce((sum, user) =>
        sum + getValidatedBalance(user.balance), 0);
};
...
const duplicateNames = (users) => {
    let arrDuplicateNames = []

    users.forEach((user, index) => {
        if (users.findIndex(obj => obj.name === user.name) !== index && !arrDuplicateNames.some(obj => obj.name === user.name)) {
...
console.log(duplicateNames(users));
