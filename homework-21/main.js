...
function average(mixed) {
    const numbers = mixed.filter(item => !isNaN(item)).map(item => Number(item));
    console.log(`новий масив після приведення: ${numbers}`);
    const sum = numbers.reduce((sumItems, item) => sumItems + item, 0);
